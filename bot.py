...
=== ВЕТРЯНОЙ ТРЕК (июль 2026) ===
Новые deep-links:
  t.me/<bot>?start=wind_ru   — переход из презентации «Ветроэнергетика»
  t.me/<bot>?start=rosatom   — переход по QR/ссылке из письма (Росатом)
  t.me/<bot>?start=nda_wind  — кнопка «NDA и переговоры» ветряного трека

Новые (необязательные) параметры в config.py:
  WIND_TEASER_FILE = "materials/Tizer_EC_VEU.pdf"   # PDF-тизер ВЭУ; "" = не слать
  NDA_FILE         = ""                             # проект NDA; "" = не слать
Если параметров нет в config — бот работает как раньше, ничего не ломается.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# deep-link аргумент -> человекочитаемый источник лида
WIND_STARTS = {
...
async def _send_wind_teaser(chat_id: int):
    """Отправляет PDF-тизер ВЭУ, если файл задан в config и существует."""
...
async def _send_nda_draft(chat_id: int):
    """Отправляет проект NDA, если файл задан в config и существует."""
...
async def _notify_wind_lead(user: types.User, source_label: str, is_nda: bool):
    """Уведомление инженерам о новом ветряном лиде (без постановки чата на паузу)."""
    uname = f"@{user.username}" if user.username else "(без username)"
    fullname = user.full_name or "—"
    client_info[user.id] = {"name": fullname, "username": uname}
...
async def cmd_start(message: Message, command: CommandObject):
    ...
    if arg in WIND_STARTS:
        ...
        await _notify_wind_lead(
            message.from_user, WIND_STARTS[arg], is_nda=(arg == "nda_wind")
        )
        return

    # ── Общий трек (как раньше): язык из deep-link или из клиента ─────────
    if arg in GREETINGS:
        lang = arg
    else:
